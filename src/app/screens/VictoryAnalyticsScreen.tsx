    ...
    <div className="h-screen bg-[#0a0515] relative overflow-hidden flex flex-col">
      ...
      <div className="relative z-10 py-4 px-6 flex-shrink-0">
        ...
        >
          ...
          <motion.p
            className="text-white/80 text-xl uppercase tracking-widest flex items-center justify-center gap-4"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: 0.5 }}
          >
            <span>Stage {matchData.stageNumber}: {matchData.stageName}</span>
            <span className="w-1 h-1 bg-white/20 rounded-full" />
            <span className={matchData.hand?.toLowerCase() === 'left' ? 'text-[#00f0ff]' : 'text-[#ffaa00]'}>
              {matchData.hand?.toUpperCase() || 'RIGHT'} HAND
            </span>
          </motion.p>
        </motion.div>
      </div>
